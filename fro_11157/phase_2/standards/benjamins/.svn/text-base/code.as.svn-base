...
// headline, text1, text2, text3, text4
var _pauseTimes:Array = [2, .25, .2, .15, .5];
...
function animateFrame3():void
{
	switch(bannerConfig.id)
	{
		...
		case "468x60":
		case "728x90":
			headline.text2.alpha = 1
		break;
	};
};


function animateFrame4():void
{
	switch(bannerConfig.id)
	{
		case "160x600":
		...
		case "468x60":
		case "728x90":
			headline.text3.alpha = 1;
		break;
	};
};


function animateFrame5():void
{
	switch(bannerConfig.id)
	{
		case "160x600":
		...
		case "468x60":
		case "728x90":
			headline.text4.alpha = 1;
		break;
	};
};


function animateFrame6():void
{
	switch(bannerConfig.id)
	{
		case "160x600":
		...
		case "300x600":
			cta.alpha = 1;
			var dist:String = String(Math.ceil(bannerConfig.dist / bannerConfig.numLines));
			TweenLite.from(cta, SPEED, { y:dist, alpha:0, ease:EASE, onComplete:shimmerCta, onCompleteScope:this });
		break;
		...
		case "468x60":
			TweenLite.to(logo, SPEED, { x:351, y:8, ease:EASE });
			cta.alpha = 1;
