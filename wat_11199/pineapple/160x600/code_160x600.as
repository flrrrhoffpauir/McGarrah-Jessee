...
{
	food_mask.visible = stroke0.visible = stroke1.visible = pie.visible = header.visible = bg.visible = shake.visible = footer.visible = cta.visible = false;
};


function initBanner():void
{
	food_mask.visible = stroke0.visible = stroke1.visible = pie.visible = header.visible = bg.visible = shake.visible = footer.visible = cta.visible = true;
	
	TweenLite.to(logo, SPEED, { x:logo.ox, y:logo.oy, scaleX:logo.os, scaleY:logo.os, delay:.5, ease:Quart.easeInOut });
	TweenLite.from(bgmask, SPEED, { scaleY:0, delay:.85, ease:EASE });
	TweenLite.from(cta, SPEED, { y:stage.stageHeight+10, delay:.85, ease:EASE });
	
	TweenLite.from(food_mask, SPEED, { scaleY:0, delay:1, ease:EASE });
	TweenLite.from(stroke0, SPEED, { y:384, delay:1, ease:EASE });
	TweenLite.from(stroke1, SPEED, { y:384, delay:1, ease:EASE });
	TweenLite.from(pie, 12.5, { scaleX:1, scaleY:1, delay:1.5, ease:Sine.easeOut });
...
function initHeaderCopy(n:Number):void
{
	...
	switch(n)
	{
		...
		case 2:
			...
			TweenLite.from(shake, 1.8, { x:-shake.width, y:"40", delay:dlay+.4, ease:EASE });
			initFooter(dlay+.4);
...
{
	if(footer.shimmer.x >= stage.stageWidth - 1)
	{
		footer.shimmer.x = 160;
...
{
	TweenLite.from(footer.mask0, SPEED, { scaleX:0, delay:dlay, ease:EASE });
	TweenLite.from(footer.mask1, SPEED, { scaleX:0, delay:dlay, ease:EASE });
	TweenLite.from(footer.copy, SPEED, { scaleX:.5, scaleY:.5, alpha:0, delay:dlay+.2, ease:Back.easeOut });
	TweenLite.from(footer.bg, .5, { alpha:0, delay:dlay, ease:EASE });
	shimmerFooter(dlay+.5);
};
